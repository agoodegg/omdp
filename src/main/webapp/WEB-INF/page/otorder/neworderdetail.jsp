...
function checkDetail(){
	if(Validator.Validate($('#form1')[0], 2)){
		$("body").mask("信息提交中...");
		$.ajax({
		    url: '<%=path %>/order/ot-newOrder/saveOrderDetail.htm',
		    type: 'POST',
		    timeout: 2000,
		    	...
		    	if(json){
		    		try{
		    			...
		    			if(jsondata.success===true){
			    			...
			    			window.location.replace("<%=path %>/order/ot-newOrder/toNewOrderDetail.htm?sec=${sec}&busiType=${busiType}");
				    		//alert(jsondata.msg);
				    	}
...
function del(id){
	...
	$("body").mask("信息提交中...");
	$.ajax({
	    url: '<%=path %>/order/ot-newOrder/delOrderDetail.htm',
	    type: 'POST',
	    timeout: 2000,
	    data:{'id':id}, 
	    error: function(){
...
function editDetail(){
	if(Validator.Validate($('#form1')[0], 2)){
		$("body").mask("信息提交中...");
		$.ajax({
		    url: '<%=path %>/order/ot-newOrder/updateOrderDetail.htm',
		    type: 'POST',
		    timeout: 2000,
		    data:$('#form1').serialize(), 
		    error: function(){
...
<form name="form1" id="form1" action="<%=path %>/order/ot-newOrder/toNewOrderDetail.htm" method="post" style="margin-top:0px">
