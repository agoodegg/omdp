...
<o:guard tag="OPR_000074">
function addUser(){
	var url = "<%=path%>/sys/employeeManage/toAdd.htm";
	
	var iTop = (window.screen.availHeight-650)/2;       //获得窗口的垂直位置;
  	var iLeft = (window.screen.availWidth-570)/2;           //获得窗口的水平位置;
  	
	window.open(url, 'addUserWin', 'height=650,width=570,top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=n o,status=no');
}
</o:guard>

<o:guard tag="OPR_000076">
function edit(id){
	var url = "<%=path%>/sys/employeeManage/toEdit.htm?id="+id
	
	var iTop = (window.screen.availHeight-650)/2;       //获得窗口的垂直位置;
  	var iLeft = (window.screen.availWidth-570)/2;           //获得窗口的水平位置;
  	
	window.open(url, 'editUserWin', 'height=650,width=570,top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=n o,status=no');
}</o:guard>

<o:guard tag="OPR_000075">
function markDelete(id){
	if(confirm("确认删除所选用户信息?")){
		$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path %>/sys/employeeManage/markDel.htm',
			    type: 'POST',
			    timeout: 2000,
			    data:{'id':id}, 
			    error: function(){
...
function resetPassword(id){
	if(confirm("确认重置所选用户密码?")){
		$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path %>/sys/employeeManage/resetPassword.htm',
			    type: 'POST',
			    timeout: 2000,
			    data:{'id':id}, 
			    error: function(){
...
<div id="main" class="mainPanel wd txt_left">
	<div class="titleInfo">							
			<h3 class="left">用户列表</h3><span class="addrtitle b_size"></span>					
			</div>
	<div class="queryArea">							
	<fieldset style="margin:5 10 5 10;">
	 <legend>查询区域</legend>
	 <form action="<%=path %>/sys/employeeManage/queryEmployee.htm" method="POST" id="queryForm">
	 <input type="hidden" name="pageNo" id="pageNoId" value="${page.pageNo}"/>
	 <table width="100%">
	 	<tr>
	 		<td class="labelField" width="10%">用户名称:</td><td width="23%"><input id="userName" type="text" name="userName" value="${q.userName}"/></td>
	 		<td class="labelField" width="10%">用户帐号:</td><td width="24%"><input id="userAccount" type="text" name="userAccount"  value="${q.userAccount}"/></td>
	 		<td class="labelField" width="10%"></td>
	 		<td width="34%"><o:guard tag="OPR_000074"><input type="button" value="新 增" onclick="addUser()"/>&nbsp;&nbsp;</o:guard><input type="button" value="重 置" onclick="resetForm()"/>&nbsp;&nbsp;<input type="button" value="查 询" onclick="query()"/></td>
	 	</tr>
	 </table></form>
	</fieldset>	
	</div>
			...
			<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;table-layout:fixed;">
				<tbody><tr>
					...
					<th style="width:9%;padding-left:50px">
					用户编号
					</th>
					<th style="width:8%;">
					用户名
				...
				<c:forEach items="${userList}" var="user" varStatus="status">
					<tr class="content_line <c:if test="${status.count%2==0}">even_row</c:if>" title="双击查看" onMouseOver="mouseOverFn(this)"  onmouseout="mouse_out_tr(this, '${status.count}')" onClick="select_tr(this,'${status.count}');">
						...
						<td>${user.deptCd}&nbsp;</td>
						<td>${user.age}&nbsp;</td>
						<td><c:if test="${user.idNo!='88888'}"><o:guard tag="OPR_000076"><a href="#" onclick="edit(${user.id})">修改</a>&nbsp;&nbsp;</o:guard><o:guard tag="OPR_000075"><a href="#" onclick="markDelete(${user.id})">删除</a>&nbsp;</o:guard><o:guard tag="OPR_000115"><a href="#" onclick="resetPassword(${user.id})">密码重置</a>&nbsp;</o:guard></c:if>&nbsp;</td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
