...
$(document).ready(function(){
	$("#modify_account_btn")[0].onclick=function(){
		if(Validator.Validate($('#edituserForm')[0], 2)){
			$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path%>/sys/employeeManage/updateUser.htm',
			    type: 'POST',
			    dataType: 'json',
			    timeout: 2000,
...
<div class="titleInfo">							
	<h3 class="left">编辑用户信息</h3><span class="addrtitle b_size"></span>					
</div>

<div name="status" class="content b_size"><form name="edituserForm" id="edituserForm" action="<%=path%>/sys/employeeManage/updateUser.htm" method="post" >
<div style="width:100%;">
<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;">
	<tr>
		<td align="right">用户帐号：<input type="hidden" name="id" id="id" value="${po.id}"/></td>
	...
	<tr>
		...
		<c:forEach items="${deptList}" var="dept" varStatus="status"><option value="${dept.deptCd}" <c:if test="${po.deptCd==dept.deptCd}">selected</c:if> >${dept.deptName}</option></c:forEach>
		</select>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td align="right">用户角色：<input type="hidden" name="roleIds" id="roleIds" value="${po.roleIds}"/></td>
		<td><INPUT type="text" name="roleNames" id="roleNames" value="${po.roleNames}"  readOnly style="background:#cfcfcf;width:260px;" title="分配用户角色"><input type="button" value="添加角色" onclick="openRoleWin()"/></td>
	</tr>
	<tr>
