...
$(document).ready(function(){
	$("#modify_account_btn")[0].onclick=function(){
		if(Validator.Validate($('#edituserForm')[0], 2)){
			$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path%>/sys/deptManage/updateDept.htm',
			    type: 'POST',
			    dataType: 'json',
			    timeout: 2000,
...
<div class="titleInfo">							
	<h3 class="left">编辑部门</h3><span class="addrtitle b_size">(编辑组织部门信息)</span>					
</div>

<div name="status" class="content b_size"><form name="edituserForm" id="edituserForm" action="<%=path%>/sys/deptManage/updateDept.htm" method="post" >
<div style="width:100%;">
<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;">
	<tr>
		<td align="right">所属父级组织：<input type="hidden" name="id" value="${po.id}"/></td>
		<td>
		<select name="pid" id="pid" dataType="LimitB" min="1"  max="40" msg="所属父组织(非空)">
		<c:forEach items="${deptList}" var="dept" varStatus="status"><option value="${dept.id}" <c:if test="${po.pid==dept.id}">selected</c:if> >${dept.deptName}</option></c:forEach>
		</select>
		</td>
	</tr>
	<tr>
		<td align="right">部门名称：</td>
		<td id="oldAccount"><INPUT type="text" name="deptName" id="deptName" value="${po.deptName}" dataType="LimitB" min="1"  max="40" msg="部门名称(非空,最大40位)" ></td>
	</tr>
