...
<o:guard tag="OPR_000077">
function addUser(){
	var url = "<%=path%>/sys/deptManage/toAdd.htm";
	var iTop = (window.screen.availHeight-305)/2;       //获得窗口的垂直位置;
  	var iLeft = (window.screen.availWidth-460)/2;           //获得窗口的水平位置;
  
	window.open(url, 'addUserWin', 'height=305,width=460,top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=n o,status=no');
}</o:guard>
<o:guard tag="OPR_000078">
function edit(id){
	var url = "<%=path%>/sys/deptManage/toEdit.htm?id="+id
	
	var iTop = (window.screen.availHeight-305)/2;       //获得窗口的垂直位置;
  	var iLeft = (window.screen.availWidth-460)/2;           //获得窗口的水平位置;
  	
	window.open(url, 'editUserWin', 'height=305,width=460,top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=n o,status=no');
}</o:guard>

<o:guard tag="OPR_000079">
function del(id){
	if(confirm("确认删除所选部门?")){
		$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path %>/sys/deptManage/delete.htm',
			    type: 'POST',
			    timeout: 2000,
			    data:{'id':id}, 
			    error: function(){
...
<div id="main" class="mainPanel wd txt_left">
	<div class="titleInfo">							
			<h3 class="left">部门管理</h3><span class="addrtitle b_size"></span>					
			</div>
	<div class="queryArea">							
	<fieldset style="margin:5 10 5 10;">
	 <legend>查询区域</legend>
	 <form action="<%=path %>/sys/deptManage/queryDept.htm" method="POST" id="queryForm">
	 <input type="hidden" name="pageNo" id="pageNoId" value="${page.pageNo}"/>
	 <table width="100%">
	 	<tr>
	 		<td class="labelField" width="10%">部门名称:</td><td width="23%"><input id="deptName" type="text" name="deptName" value="${q.deptName}"/></td>
	 		<td class="labelField" width="10%"></td><td width="24%"></td>
	 		<td class="labelField" width="10%"></td>
	 		<td width="34%"><o:guard tag="OPR_000077"><input type="button" value="新 增" onclick="addUser()"/>&nbsp;&nbsp;</o:guard><input type="button" value="重 置" onclick="resetForm()"/>&nbsp;&nbsp;<input type="button" value="查 询" onclick="query()"/></td>
	 	</tr>
	 </table></form>
	</fieldset>	
	</div>
			...
			<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;table-layout:fixed;">
				...
				<c:forEach items="${deptList}" var="dept" varStatus="status">
					<tr class="content_line <c:if test="${status.count%2==0}">even_row</c:if>" title="双击查看" onMouseOver="mouseOverFn(this)"  onmouseout="mouse_out_tr(this, '${status.count}')" onClick="select_tr(this,'${status.count}');">
						<td>${status.count}&nbsp;</td>
						<td>${dept.deptName}&nbsp;</td>
						<td><c:if test="${dept.id!=1}"><o:guard tag="OPR_000078"><a href="#" onclick="edit(${dept.id})">修改</a>&nbsp;&nbsp;</o:guard><o:guard tag="OPR_000079"><a href="#" onclick="del(${dept.id})">删除</a>&nbsp;&nbsp;</o:guard></c:if>&nbsp;</td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
