...
$(document).ready(function(){
	$("#modify_account_btn")[0].onclick=function(){
		if(Validator.Validate($('#edituserForm')[0], 2)){
			$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path%>/sys/roleManage/updateRole.htm',
			    type: 'POST',
			    dataType: 'json',
			    timeout: 2000,
...
<div class="titleInfo">							
	<h3 class="left">编辑角色</h3><span class="addrtitle b_size"></span>					
</div>

<div name="status" class="content b_size"><form name="edituserForm" id="edituserForm" action="<%=path%>/sys/roleManage/updateRole.htm" method="post" >
<div style="width:100%;">
<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;">
	<tr>
		<td align="right">角色名称：<input type="hidden" name="id" id="id" value="${po.id}"/><input type="hidden" name="roleId" id="roleId" value="${po.roleId}"/></td>
		<td>
		<input name="roleName" id="roleName" type="text" dataType="LimitB" min="1" max="40" value="${po.roleName}" msg="角色名称(非空,最大40位)"/>
		</td>
	</tr>
	<tr>
		<td align="right">角色描述：</td>
		<td id="oldAccount"><textarea rows="4" cols="30" name="roleDesc" id="roleDesc" dataType="LimitB" max="40" msg="角色描述(最大80位)">${po.roleDesc}</textarea> </td>
	</tr>
