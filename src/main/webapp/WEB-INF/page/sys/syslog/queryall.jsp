...
function resetForm(){
	$('#logType').val("");
	$('#orderTimePickerU').val("");
	$('#orderTimePickerD').val("");
}
...
<div id="main" class="mainPanel wd txt_left">
	<div class="titleInfo">							
			<h3 class="left">系统操作日志</h3><span class="addrtitle b_size"></span>					
			</div>
	<div class="queryArea">							
	<fieldset style="margin:5 10 5 10;">
	 <legend>查询区域</legend>
	 <form action="<%=path %>/sys/syslog/queryAllLog.htm" method="GET" id="queryForm">
	 <input type="hidden" name="pageNo" id="pageNoId" value="${page.pageNo}"/>
	 <table width="100%">
	 	<tr>
	 		<td class="labelField" width="10%">日志类型:</td><td width="23%">
	 		<o:select name="logType" id="logType" value="${log.logType}" type="LOG_OPR_TYPE"  headValue="" headText="---请选择---"></o:select>
	 		</td>
	 		<td class="labelField" width="10%">操作时间≥:</td><td width="24%">
	 		<input style="width:87px;" type="text" name="logTimeU"  value="${log.logTimeU}" onfocus="blur()" readonly="readonly" id="orderTimePickerU"/>≤<input   value="${log.logTimeD}"  onfocus="blur()" style="width:87px;" type="text" name="logTimeD" id="orderTimePickerD"/></td>
	 		<td class="labelField" width="10%"></td>
	 		<td width="23%">
	 		<input type="button" value="重 置" onclick="resetForm()"/>&nbsp;&nbsp;<input type="button" value="查 询" onclick="query()"/>
	 		</td>
	 	</tr>
	 </table></form>
	</fieldset>	
	...
	<div style="width:100%;">
			<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;table-layout:fixed;">
				<tbody><tr>
					...
					<a href="#syslog/operlog/time,asc">操作员</a>
					</th>
					<th style="width:10%;">
					<a href="#syslog/operlog/operation,asc">日志类型</a>
					</th>
					<th style="width:19%;border-right:none;">
					<a href="#syslog/operlog/operator,asc">操作时间</a>
					</th>
				...
				<c:forEach items="${logList}" var="log" varStatus="status">
					<tr class="content_line <c:if test="${status.count%2==0}">even_row</c:if>" title="双击查看" onMouseOver="mouseOverFn(this)"  onmouseout="mouse_out_tr(this, '${status.count}')" onClick="select_tr(this,'${status.count}');">
						...
						<td>${log.userName}&nbsp;</td>
						<td>
						<o:dict type="LOG_OPR_TYPE" value="${log.logType}"></o:dict>&nbsp;
						</td>
