...
var dataUrl = '<%=path%>/order/orderQuery/queryAllData.htm';
var gtype = '';
<c:choose><c:when test="${mode==1}">
gtype = '未完成-';
dataUrl = '<%=path%>/order/orderQuery/queryUnfinishData.htm';
</c:when><c:when test="${mode==2}">
gtype = '已完成-';
dataUrl = '<%=path%>/order/orderQuery/queryDoneData.htm';
</c:when><c:when test="${mode==3}">
gtype = '已作废-';
dataUrl = '<%=path%>/order/orderQuery/queryTrashData.htm';
</c:when></c:choose>
...
Ext.onReady(function(){
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        columns: [
		    ...
		    {header:'开单员',width:70,sortable:false,dataIndex:'creatorName'},<% if("queryTrash".equals(action)){%>
				...
				items:[{
	                ...
	                items:[{
			                ...
			                items:[{xtype:'label',text:'   订单号:'},{
			                    ...
			                    xtype:'datefield',id:'orderTimePickerU',width:125,format:'Y-m-d',editable:false,value:'${p.orderTimePickerU}'
			                },{xtype:'label',text:'≤'},{
