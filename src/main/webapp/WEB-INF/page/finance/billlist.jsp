...
function viewDetail(custId){
	var upTime = $.trim($("#hideUpTime").val());
	var downTime = $.trim($("#hideDownTime").val());
	
	var url = "<%=path %>/finance/bill/singleDetail.htm?uptime="+upTime+"&downtime="+downTime+"&custId="+custId;
	
	var iTop = (window.screen.availHeight-450)/2;       //获得窗口的垂直位置;
  	var iLeft = (window.screen.availWidth-850)/2;           //获得窗口的水平位置;
	window.open(url, 'viewSingleDetail', 'height=450,width=850,top='+iTop+',left='+iLeft+',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=n o,status=no');
}
</script>
</head>
<body>
<div id="main" class="mainPanel wd txt_left">
	...
	<fieldset style="margin:5 10 5 10;">
	 <legend>查询区域</legend>
	 <input type="hidden" id="hideUpTime" value="${q.uptime}">
	 <input type="hidden" id="hideDownTime" value="${q.downtime}">
...
<table cellspacing="0" align="center" class="infoTable domainInfo listTable" style="width:100%;table-layout:fixed;">
				...
				<c:forEach items="${billList}" var="order" varStatus="status">
					<tr class="content_line <c:if test="${status.count%2==0}">even_row</c:if>" onMouseOver="mouseOverFn(this)"  onmouseout="mouse_out_tr(this, '${status.count}')" onClick="select_tr(this,'${status.count}');">
						...
						<td>&nbsp;</td>
						<td><a href="#" onclick="viewDetail('${order.custId}')">查看详细</a></td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
