...
<html>
<head>
	<title>订单结算-【${order.ordernum}】</title>
...
function dopay(){
	var tradType = $("#tradType").val();
	if($.trim(tradType)=='1'){
		if(!(confirm("月结客户！确认现在结算?"))){
			return false;
		}
	}
	
	
	if(Validator.Validate($('#payForm')[0], 2)){
		if($("#feeType").val()=='2'){
			if($.trim($("#checkNum").val())==''){
				alert("支票结算必须填写支票号!")
				return;
			}
		}
		...
		var feeTypeCn = "";
		var feeTypeOptions = $("#feeType")[0].options;
		for(var i=0;i<feeTypeOptions.length;i++){
			if(feeTypeOptions[i].value==feeTypeVal){
				feeTypeCn = feeTypeOptions[i].text;
			}
		}
		
		var confirmMsg = [];
		confirmMsg.push("订单结算操作确认\n-----------------------------------------------------------------------\n\n")
		confirmMsg.push("单号:"+$("#ordernum").val()+"\n");
		
		confirmMsg.push("客户名:"+$("#custName").val()+"\n");
		confirmMsg.push("结算方式:"+feeTypeCn+"\n");
		confirmMsg.push("应收:"+$("#yfee").val()+"\n")
		confirmMsg.push("实收:"+$("#acFee").val()+"\n")
		if($("#feeType").val()=='2'){
			confirmMsg.push("支票号:"+$("#checkNum").val()+"\n")
		}
		if($("#checkStatus").val()=='1'){
			confirmMsg.push("已开发票")
		}
		else{
			confirmMsg.push("未开发票")
		}
		...
		if(confirm(confirmMsg.join(""))){
			...
			$("body").mask("信息提交中...");
			$.ajax({
			    url: '<%=path %>/finance/trad/dopayorder.htm',
			    type: 'POST',
			    timeout: 2000,
...
<body topmargin="0">
<input type="hidden" id="tradType" value="${order.ext1}">
...
<tr>
	...
	<tr>
		...
		<table width="100%" border="1" align="center" style="border-collapse:collapse;border-top-style:none" bordercolor="#000000" >
			...
			<tr align="left" valign="top">
				...
				<td align="center" nowrap style="font-size:13px;">${detail.amount}${detail.unit}</td>
				<td align="center" style="font-size:13px;">${detail.unitPrice}</td>
				<td align="right" style="font-size:13px;font-weight:bold;color:#ff0000;">${detail.total}</td>
	...
	<tr>
		<td></td>
		<td height="30" style="font-size:16px;color:#ff0000;font-weight:bold;" align="right" colspan="5">合计:&nbsp;${order.price} 元  
		</td>
	</tr>
</table>
...
<form id="payForm" action="<%=path%>/finance/trad/dopayorder.htm" method="POST">
...
<tr>
		...
		<td width="17%">${order.price} 元<input type="hidden" id="totalPrice" value="${order.price}"/>
		</td>
	...
	<tr>
		...
		<td width="17%"><o:select type="payType" id="feeType" name="feeType" headText="---请选择---" headValue="" dataType="LimitB" min="1"  max="40" msg="结算方式(非空)"></o:select>
		</td>
		<td width="15%" align="center" style="color:#0000ff;">应收：</td>
		<td width="17%"> <font color="#ff0000">${order.price} 元</font><input id="yfee" name="yfee" value="${order.price}" type="hidden"  dataType="Currency" min="1"  max="40" msg="应收金额(非空,数值型)"/> 
		</td>
		<td width="15%" align="center" style="color:#0000ff;" >实收：</td>
		<td><input type="text" name="acFee" id="acFee" onKeyPress="inputNumber();"  onbeforepaste="pasteNumber();" dataType="Currency" min="1"  max="40" msg="实收金额(非空,数值型)"/></td>
	</tr>
	<tr>
		<td width="15%" align="center">支票号：</td>
		<td><input type="text" name="checkNum" id="checkNum" dataType="LimitB"   max="20" msg="支票号(最大长度20)"/></td>
		<td width="15%" align="center" nowrap>是否已开发票：</td>
		<td width="17%"><o:select type="sendBill" id="checkStatus" name="checkStatus"  headText="---请选择---" headValue=""></o:select>
		</td>
		<td width="15%" align="center">发票类型：</td>
		<td><o:select type="billType" name="checkType" id="checkType" headText="---请选择---" headValue=""></o:select></td>
	</tr>
	<tr>
		<td width="15%" align="center">结算备注：</td>
		<td colspan="5" height="50">
		<textarea name="tradComm" style="width:100%;height:100%;" dataType="LimitB"   max="100" msg="结算备注(最大长度100)"></textarea>
		</td>
	</tr>
	<tr>
		<td colspan="6" align="center"><input type="button" onclick="closePayWin()" value="取 消"/>&nbsp;&nbsp;&nbsp;<input type="reset" value="重 置"/>&nbsp;&nbsp;&nbsp;<input type="button" value="结 算" onclick="dopay()"></td>
	</tr>
</table>
</form>
</body>
